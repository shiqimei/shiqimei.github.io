---
title: "The New Engine: The Living Kernel"
date: 2026-02-24
excerpt: "The context protocol described the water. The bootstrap spiral described the motion. Neither exists without the pipe — a versioned, governed, replayable runtime where context is the primary citizen and agents are ephemeral compute. ContextOS is that pipe. And its deepest proof of concept is that it builds itself."
---
...
[Part I](/posts/end-of-human-bandwidth) argued that compensatory structures evaporate when agents replace humans on the main execution path. [Part II](/posts/the-context-protocol) argued that context infrastructure fills the vacuum — the protocol that lets agents accumulate intelligence across sessions. [Part III](/posts/the-bootstrap-spiral) argued that agents use that infrastructure to bootstrap recursively, coordinating through the substrate and grounded by test suites.
...
1. Agent calls `propose_patch(operations, causal_refs)`
2. Governance validates: schema check, invariant enforcement, conflict detection
3. If valid: kernel commits to DAG
4. If conflict: kernel creates a resolution node — a first-class record that a conflict occurred and how it was resolved
...
1. Compute k-hop structural closure from focus node
2. Rank semantic neighbors (vector index when available, structural distance as fallback)
3. Enforce diversity constraint (prevent over-concentration on one subgraph)
4. Attach skill manifest filtered by agent role
5. Apply budget trimming (token budget, node budget, uncertainty ceiling)
6. Freeze view, compute hash, record in trace
...
1. **Closed-loop throughput**: every run produces usable memory or patches, not noise
2. **Replayability**: any commit can explain why the system looks the way it does now
3. **Pollution control**: invalid patches are rejected before reaching the main line
...
> Watcher triggers &rarr; spawn agent &rarr; inject view &rarr; agent produces patch &rarr; governance validates &rarr; kernel commits &rarr; experience extracted &rarr; influences next injection &rarr; loop
...
- [The New Engine: The Bootstrap Spiral](/posts/the-bootstrap-spiral) — on recursive self-improvement and tests as governance
- [ContextOS Specification](https://github.com/anthropics/contextos) — the full engineering specification
...
[第一篇](/posts/end-of-human-bandwidth)论证了当智能体取代人类进入主执行路径时，补偿结构会蒸发。[第二篇](/posts/the-context-protocol)论证了上下文基础设施填补了真空——让智能体跨会话积累智慧的协议。[第三篇](/posts/the-bootstrap-spiral)论证了智能体利用这些基础设施递归自举，通过基底协调，以测试为支撑。
...
不是数据库。不是文档存储。不是键值缓存。基底是一个**版本化上下文图** (Versioned Context Graph)：节点代表信念、目标、技能、痕迹和记忆；边代表论证、分解、因果引用和所有权。对这个图的每一次变更都走同一条路径：智能体提出补丁 (Patch)，治理层验证，内核提交到追加写入的 DAG。
...
图在提交层面是追加写入的。智能体永远不直接修改它。它们提出**补丁**——一组操作（添加节点、更新节点、添加边、删除边）——内核要么提交，要么拒绝。每个已提交的状态都是快照。每个快照都可复现。检出任意提交，就能得到那个时刻的精确图。同一提交，同一视图，同一决策。
...
1. 智能体调用 `propose_patch(operations, causal_refs)`
2. 治理层验证：模式检查、不变量执行、冲突检测
3. 若通过：内核提交到 DAG
4. 若冲突：内核创建决议节点 (Resolution Node)——一条冲突发生及其解决方式的一等记录
...
五条不变量。五面承重墙。它们不让系统变聪明。它们让系统*不发疯*。
...
1. 从焦点节点计算 k 跳结构闭包
2. 排列语义邻居（有向量索引用向量索引，否则以结构距离兜底）
3. 施加多样性约束（防止过度集中在某个子图）
4. 按智能体角色挂载技能清单
5. 执行预算裁剪（token 预算、节点预算、不确定度上限）
6. 冻结视图，计算哈希，写入痕迹
...
1. **闭环吞吐**：每次运行产出可用的记忆或补丁，而非噪声
2. **可重放性**：任意提交都能解释「系统为什么现在长这样」
3. **污染可控**：无效补丁在到达主线前被拦截
...
> 触发 &rarr; 生成智能体 &rarr; 注入视图 &rarr; 智能体产出补丁 &rarr; 治理验证 &rarr; 内核提交 &rarr; 提取经验 &rarr; 影响下次注入 &rarr; 循环
...
- [新引擎：自举螺旋](/posts/the-bootstrap-spiral)——关于递归自我改进与测试即治理
- [ContextOS 规格](https://github.com/anthropics/contextos)——完整工程规格
